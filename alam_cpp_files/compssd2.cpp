...
void CompSSD2(double *S, const double *X, int mX, int nX, const double *Y, int mY, int nY,
         ...
         if (IxC_i != -2147483647 && IxR_i != -2147483647) { // Intersection between X and Y exists?
            ...
            for (m=0; m<=ItsctLenR-1; m++)
               for (n=0; n<=ItsctLenC-1; n++)
                  dsum += square(X[(IxR_i + m)+(IxC_i + n)*mX] 
                              - Y[(IxR_i + m - k)+(IxC_i + n - l)*mY]); // SUM-SQUARE-DIFFERENCE at delay k
...
void mexFunction(int nlhs, 
   ...
   S = mxGetPr(plhs[0]);
   CompSSD2(S, X, mX, nX, Y, mY, nY, int(MXR), int(MXC), int(MNR), int(MNC));
}
